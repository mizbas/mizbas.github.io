 ...
 */
 const TO_DO_APP = () => {
  ...
  const renderTask = () => {
      ...
      const dataAll = model.getItem();
      if (dataAll.length <= 0) {
          stage.innerHTML = '<div class="alert alert-info" role="alert">下記フォームから商品名を入力してください</div>';
          return;
      }
  ...
  const setDeleteAllTask = () => {
      ...
      deleteBtn.addEventListener('click', () => {
          const res = confirm('本当に全商品を削除して良いですか？ ※復元できません');
          if (res) {
              model.deleteAllItem();
  ...
  const showFirstMainContent = () => {
      if (lStorage.app && lStorage.app !== '[]') {
          ...
          html += '<div class="alert alert-info" role="alert">下記フォームから商品名を入力してください</div>';
